...
def purchase_type(main_config, in_config, present_quarter_pd):
    try:
        ...
        except Exception as output_file_read_exception:
            exception_message = "Error occurred while opening output file for formatting from path {0} \n\t {1}".format(main_config["Output_File_Path"], output_file_read_exception)
            raise BusinessException(exception_message)
        # Format Q4 & Q3
        for cell in ws['B']:
            cell.number_format = "#,###,##"
        ...
        for c in ascii_lowercase:
            ws[c + "17"].fill = fill_solid_light_blue
            if c == 'c':
                break

        # Footer Fill
        for c in ascii_lowercase:
            ws[c + str(m_row)].fill = fill_solid_light_blue
            if c == 'c':
                break
